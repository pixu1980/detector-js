(function (global, factory) {
  ...
  function (_FlagsClass) {
    ...
    _createClass(Browsers, [{
      key: "AndroidBrowser",
      get: function get() {
        if (Asserts.all([!/like android/i.test(this._ua), /android/i.test(this._ua), /(?:chrome|crios|crmo)\/(\d+(\.?_?\d+)+)/i.test(this._ua)])) {
      ...
      key: "Electron",
      get: function get() {
        return Asserts.one([// Renderer process
        typeof window !== 'undefined' && _typeof(window.process) === 'object' && window.process.type === 'renderer', // Main process
        typeof process !== 'undefined' && _typeof(process.versions) === 'object' && !!process.versions.electron, // Detect the user agent when the `nodeIntegration` option is set to true
        _typeof(window.navigator) === 'object' && typeof this._ua === 'string' && this._ua.indexOf('Electron') >= 0]);
      }
    }, {
      key: "IE",
      get: function get() {
        return Asserts.one([/(msie |rv:)(\d+(\.\d+)?)/i.test(this._ua), /(msie|trident)/i.test(this._ua)]);
      }
    }, {
      key: "Chrome",
      get: function get() {
        if (Asserts.all([/(?:chrome|crios|crmo)\/(\d+(\.?_?\d+)+)/i.test(this._ua), !this.AndroidBrowser])) {
          this._version = RegExp.$1;
          return true;
        }
      ...
      key: "Firefox",
      get: function get() {
        return Asserts.one([/firefox/i.test(this._ua)]);
      }
    }, {
      key: "Opera",
      get: function get() {
        if (Asserts.one([/(?:opera)[\s/](\d+(\.?_?\d+)+)/i.test(this._ua), /(?:opr|opios)[\s/](\S+)/i.test(this._ua)])) {
          this._version = RegExp.$1;
          return true;
        }
      ...
      key: "PhantomJS",
      get: function get() {
        return Asserts.one([/phantom/i.test(this._ua)]);
      }
    }, {
      key: "Safari",
      get: function get() {
        return Asserts.one([/safari|applewebkit/i.test(this._ua), /safari/i.test(this._ua) && !this.Chrome && !this.PhantomJS]) && Asserts.all([!this.AndroidBrowser]);
      }
    }, {
      key: "SafariMobile",
      get: function get() {
        return Asserts.one([/iphone|ipad|ipod/i.test(this._ua), /safari/i.test(this._ua) && !this.Chrome && !this.PhantomJS && !this.AndroidBrowser]);
      }
    }, {
      key: "Yandex",
      get: function get() {
